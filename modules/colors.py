...
class Colors(commands.Cog):
    ...
    async def color_mode_status(self, ctx: commands.Context):
        ...
        await ctx.send(f":art: Color mode is currently set to {mode.title()}")
    ...
    async def color_mode(self, ctx: commands.Context, new_mode: str = None):
        ...
        if new_mode == current_mode:
            await ctx.send(f"Color system is already {current_mode.title()}")

        elif new_mode not in ('disabled', 'communal', 'personal'):
            await ctx.send("Invalid option given. Valid options are: `disabled`, `communal`, `personal`.")

        else:
            await self.bot.db.execute("UPDATE color_settings SET mode = $1 WHERE guild_id = $2", new_mode, ctx.guild.id)
            await ctx.send(f"Color mode now set to {new_mode.title()}")
    ...
    async def central_color(self, ctx: commands.Context, *, color: str = None):
        ...
        if current_system == 'communal':
            ...
            await ctx.send(f"Color system is disabled for {ctx.guild.name}")
    ...
    async def toggle_communal_color(self, ctx: commands.Context, keyword: str):
        ...
        if role in author.roles:
            try:
                ...
                return await ctx.send("Unable to manage roles.")

            return await ctx.send(f"Removed {role.name}")
        ...
        await ctx.send(f"Color switched to {role.name}")
    ...
    async def toggle_personal_color(self, ctx: commands.Context, color_hex_in: str, user: discord.Member = None):
        ...
        if not color_data:
            ...
            if not rgb_triple:
                return await ctx.send("Invalid hex or RGB value.")
            else:
        ...
        if not role_data:
            ...
            try:
                ...
                return await ctx.send("Unable to manage roles.")
            await self_roles.add_self_role(ctx, 'personal_colors', color_role, user_id=user.id, color_hex=color_data[1])
            await user.add_roles(color_role, reason="Personal color role added.")
            await ctx.send(f"Role created with hex `{color_data[1]}` and added to you.")
            ...
            if role_data[1] != color_data[1]:
                ...
                await ctx.send(f"Updated color to `{color_data[1]}`.")
            else:
                await ctx.send("Color role hex is the same.")
    ...
    async def add_color_role(self, ctx: commands.Context, identifier: typing.Union[discord.Member, str],
        ...
        if mode == 'communal':
            ...
            "add", "manualadd", "remove", "del", "delete", "list", "info", "removeall", "delall", "deleteall"):
                return await ctx.send(f"Cannot use {identifier} as a keyword. A command has this name.")
            color_tuple = common.hex_to_color(color_string)
            if not color_tuple:
                return await ctx.send("Invalid hex given.")
            else:
                color_hex = color_tuple[1]
            await self_roles.add_self_role(ctx, 'communal_colors', role, keyword=identifier, color_hex=color_hex)
            return await ctx.send(f"Communal color {role} set with color hex `{color_hex}`.")
        elif mode == 'personal':
            if isinstance(identifier, str):
                return await ctx.send("Invalid user, please pass in a member of the server to create a role for.")
    ...
    async def remove_color_role(self, ctx: commands.Context, identifier: typing.Union[discord.Member, str]):
        ...
        if mode == "communal":
            if not isinstance(identifier, str):
                return await ctx.send("Invalid keyword given")
            if await self_roles.delete_self_role(ctx, 'communal_colors', ('keyword', identifier)) == -1:
                return
            await ctx.send(f"Communal color {identifier} removed")
            ...
            if await self_roles.delete_self_role(ctx, 'personal_colors', ('user_id', identifier.id)) == -1:
                return
            await ctx.send(f"Personal color role for {identifier} removed.")
        else:
            await ctx.send("Color system disabled, please enable to use this command.")
    ...
    async def remove_all_colors(self, ctx: commands.Context):
        ...
        mode = await self.bot.db.fetchval("SELECT mode FROM color_settings WHERE guild_id = $1", ctx.guild.id)
        if mode == 'disabled':
            return await ctx.send("Color system disabled, please enable to use this command.")
        table = mode + "_colors"
    ...
    async def color_info(self, ctx: commands.Context, identifier: typing.Union[discord.Member, str] = None):
        ...
        if mode == "communal":
            if not identifier:
                return await ctx.send_help(ctx.command)
            if not isinstance(identifier, str):
                return await ctx.send("Invalid keyword")
