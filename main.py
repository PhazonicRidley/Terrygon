...
from utils import logger as discord_logger, errors
import json
...
class Terrygon(commands.Bot):
    def __init__(self):
        ...
        super().__init__(command_prefix=_callable_prefix, description=read_config("description"), max_messages=10000, help_command=help_cmd,
                         allowed_mentions=discord.AllowedMentions(everyone=False, users=True, roles=True))

        try:
            self.db = loop.run_until_complete(create_pool())
        except Exception:
    ...
    async def on_command_error(self, ctx, error):
        ...
        # handles all uncaught http connection failures.
            ...
            await ctx.send(f"An HTTP {error.original.status} error has occurred for the following reason: `{error.original.text}`")

        # handles all bad command usage
        elif isinstance(error, (commands.MissingRequiredArgument, commands.BadArgument, commands.BadUnionArgument, commands.TooManyArguments, flags.ArgumentParsingError)):
            await ctx.send_help(ctx.command)

        # handles command cool downs
        elif isinstance(error, commands.errors.CommandOnCooldown):
            await ctx.send("This command was used {:.2f}s ago and is on cooldown. Try again in {:.2f}s.".format(error.cooldown.per - error.retry_after, error.retry_after))
    ...
    async def is_log_registered(self, guild: discord.Guild, log_type):
        ...
        async with self.db.acquire() as conn:
            log_channel_types = await conn.fetchrow("SELECT column_name FROM information_schema.columns WHERE table_name = 'log_channels' AND column_name != 'guildid';")
            i = 0
